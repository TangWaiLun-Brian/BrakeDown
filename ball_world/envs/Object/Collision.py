import random
def check_collison(obstacle, ball, previous, current):
    """flag = -1
    if obstacle.rect.left <= ball.rect.right and obstacle.rect.right >= ball.rect.left and obstacle.rect.top <= ball.rect.centery <= obstacle.rect.bottom:
        if previous != current:
            ball.speed[0] *= -1
            ball.speed[1] += random.uniform(-1, 1)
        flag = current
    ...
    if obstacle.rect.top <= ball.rect.bottom and obstacle.rect.bottom >= ball.rect.top and obstacle.rect.left <= ball.rect.centerx <= obstacle.rect.right:
        if previous != current+2:
            ball.speed[1] *= -1
            ball.speed[0] += random.uniform(-1, 1)
        flag = current+2
    ...
    if obstacle.rect.colliderect(ball.rect):
        if abs(obstacle.rect.top - ball.rect.bottom) < tolerance and ball.speed[1] > 0:
            ball.speed[1] *= -1
            ball.speed[0] += random.uniform(-0.1, 0.1)
            flag = current
        ...
        if abs(obstacle.rect.left - ball.rect.right) < tolerance and ball.speed[0] > 0:
            ball.speed[0] *= -1
            ball.speed[1] += random.uniform(-0.1, 0.1)
            flag = current + 2
...
def ball_collide_with_obstacles(ball, obstacles, previous_collison):
    tmp_pre = -1
    for i, obstacle in enumerate(obstacles):
        collide = check_collison(obstacle, ball, previous_collison, i * 4)
        if collide >= 0:
            tmp_pre = collide
            break
